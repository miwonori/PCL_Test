...
struct Ray {
    ...
    // resoultion: the magnitude of the ray's step, used for ray casting, the smaller
    ...
    void rayCast(const std::vector<Car>& cars, double minDistance,
        ...
        while(!collision &&castDistance < maxDistance) {
            ...
            // check if there is any collision with cars
...
struct Lidar {
    ...
    Lidar(std::vector<Car> setCars, double setGroundSlope) :
        ...
        maxDistance = 50;
        resolution = 0.2;
        // TODO : set sderr to 0.2 to get mor interesting pcd files
        sderr = 0.05;
        cars = setCars;
        ...
        for(double angleVertical = steepestAngle;
            ...
            for(double angle = 0.0; angle <= 2*pi; angle += horizontalAngleInc) {
    ...
    ~Lidar(){
        // pcl uses boost smart pointers for cloud pointer so we don't have
        // to worry about manually freeing the memory
    }
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr scan() {
        ...
        auto elapseTime = std::chrono::duration_cast<std::chrono::milliseconds>
                (endTime-startTime);
        cout << "ray casting Took" << elapseTime.count() << " milliseconds"
        << endl;
